...
public final class CameraManager {
	...
	private static final int CAPTURE_HEIGHT = 320;

	private static final int CAPTURE_WIDTH = 600;
	static {
		int sdkInt;
		try {
	...
	public void openDriver(SurfaceHolder holder) throws IOException {
		if (camera == null) {
			...
			Log.i(TAG,fr.toShortString());
			if (!initialized) {
				initialized = true;
				configManager.initFromCameraParameters(camera,fr);
			}
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			height=CameraManager.CAPTURE_HEIGHT; width=CAPTURE_WIDTH;
	...
	public Rect getFramingRectInPreview() {
		if (framingRectInPreview == null) {
			...
			if( !CameraManager.isLandscape){
				...
				int i = cameraResolution.x;cameraResolution.x=cameraResolution.y;cameraResolution.y=i;
	...
	public static void setCameraDisplayOrientation(Activity activity) {
		...
		if ( CameraManager.isLandscape==true)
			return;
		...
		CameraManager.get().camera.setDisplayOrientation(result);
	}
