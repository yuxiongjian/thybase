...
public final class PlanarYUVLuminanceSource extends LuminanceSource {
	...
	public Bitmap renderCroppedGreyscaleBitmap() {
		...
		return renderCroppedGreyscaleBitmap(yuvData, dataWidth, dataHeight,
				top, left, width, height);
	}

	public Bitmap renderCroppedGreyscaleBitmapAll() {
		int width = getWidth();
		int height = getHeight();
		int dw=dataHeight;
		int dh=dataWidth;
		...
		if (!CameraManager.isLandscape) {
			...
			for (int y = 0; y < dh; y++) {
				...
				for (int x = 0; x < dw; x++)
					rotatedData[x * dh + dh - y - 1] = yuvData[x
							+ y * dw];
			}

			//return renderCroppedGreyscaleBitmap(rotatedData, dataWidth,dataHeight,0,0,dataWidth,dataHeight);
			return renderCroppedGreyscaleBitmap(rotatedData, dataWidth,dataHeight, top, left, width*15/10, height*15/10);
		}
		else*/
		return renderCroppedGreyscaleBitmap(yuvData,  dataWidth,dataHeight,
	...
	public Bitmap renderCroppedGreyscaleBitmap(byte[] yuv, int dataWidth,
